...
val statement = if (collection == "traffic") {
  ...
  "select year, week, sum(pax) as cnt, first_value(origin_iso), origin_id, first_value(dest_iso), dest_id from mobils group by origin_id, dest_id, year, week order by cnt"
}

...
var localZippedDir = local_store + config.localZippedDir;
var processed_dir = local_store + config.localProcessedDir;
  ...
  return new Promise(function(resolve, reject) {
    ...
    fs.writeFile(local_dir + file, db_fields + '\n', (err) => {
      ...
      fs.readdir(path, (err, files) => {
        bluebird.map(files.filter(function(e) {
          return e.match(/csv$/);
        }), function(f) {
...
exports.unzip = function(dir, file) {
  ...
  return new Promise(function(resolve, reject) {
    if (file.match(/csv$/)) {
      ...
      exec('gunzip ' + localZippedDir + file, function(err, out, code) {
        if (err) {
